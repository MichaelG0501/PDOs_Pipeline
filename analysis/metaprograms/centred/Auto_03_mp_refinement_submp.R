...
#     heatmap. Adapted from scRef centred pipeline.
#
#   Inputs:
...
#       refined_mp_correlation_heatmap_unsupervised.pdf
#       refined_mp_jaccard_heatmap.pdf
#   Outputs (ephemeral: PDOs_outs/centred_mp_refinement/intermediate/):
...
cat(paste0("Loading nMP", optimal_nMP, " metaprograms object...\n"))
...
# ============================================================================
# 2. GeneNMF helper functions (local re-implementations)
# ============================================================================
...
# Data-driven grouping (no hardcoded state assignments)
split_mps_for_final  <- split_mps
keep_mps_for_final   <- keep_mps

cat("\nKeep MPs :", paste(keep_mps_for_final, collapse = ", "), "\n")
cat("Split MPs:", paste(split_mps_for_final, collapse = ", "), "\n")

# ============================================================================
# 4. Sub-splitting
# ============================================================================

cached_split <- file.path(outdir_ephemeral, "split_results.rds")
split_results <- NULL
...
write.csv(split_selection_summary,
          file.path(outdir_live, "tables", "split_selection_summary.csv"),
          row.names = FALSE)
cat("Saved: tables/split_selection_summary.csv\n")

# ============================================================================
# 5. Sub-MP gene list derivation (replicates GeneNMF consensus approach)
# ============================================================================

cached_genes <- file.path(outdir_ephemeral, "refined_mp_genes.rds")
cached_gene_weights <- file.path(outdir_ephemeral, "refined_mp_gene_weights.rds")
...
if (!replot_only && (force_rebuild || !gene_cache_valid)) {
  ...
  saveRDS(assignments, file.path(outdir_ephemeral, "refined_mp_assignments.rds"))
...
# ============================================================================
# 6. Build refined MP set for final plots (data-driven)
# ============================================================================

refined_mps_ordered <- character(0)
for (mp in mp_names_all) {
  if (mp %in% remove_mps) next
  if (mp %in% keep_mps) {
    if (mp %in% names(refined_mp_genes)) {
      refined_mps_ordered <- c(refined_mps_ordered, mp)
    }
  } else if (mp %in% split_mps) {
    res <- split_results[[mp]]
    if (is.null(res) || res$skipped) {
      if (mp %in% names(refined_mp_genes)) {
        refined_mps_ordered <- c(refined_mps_ordered, mp)
      }
    } else {
      sub_labels <- sort(unique(res$sub_labels))
      sub_labels <- sub_labels[sub_labels %in% names(refined_mp_genes)]
      refined_mps_ordered <- c(refined_mps_ordered, sub_labels)
    }
  }
}

cat("\n=== Refined MP Order for Final Plots ===", "\n")
cat("  ", paste(refined_mps_ordered, collapse = ", "), "\n")

# Subset gene lists, drop empty
...
mp_display_labels <- setNames(refined_mps_ordered, refined_mps_ordered)
...
write.csv(refined_gene_size_table,
          file.path(outdir_live, "tables", "refined_mp_gene_sizes.csv"),
          row.names = FALSE)
...
cached_ucell <- file.path(outdir_ephemeral, "refined_ucell_scores.rds")
...
if (force_rebuild || !ucell_cache_valid) {
  cat("\nLoading PDOs merged Seurat object...\n")
  ...
  pdos_merged <- AddModuleScore_UCell(pdos_merged, features = refined_mp_genes,
                                       ncores = 1, name = "")
  refined_ucell <- pdos_merged@meta.data[, names(refined_mp_genes), drop = FALSE]
  ...
  cat("Loading PDOs merged Seurat object (for sample metadata)...\n")
  pdos_merged <- readRDS(file.path(live_base, "PDOs_merged.rds"))
  pdos_merged <- subset(pdos_merged, subset = orig.ident != "SUR843T3_PDO")
}
...
pdf(file.path(outdir_live, "figures", "mp_splitting_diagnostics.pdf"),
...
for (mp in split_mps) {
  ...
  # NMF program similarity heatmap
  J_sub <- res$J_sub
  ...
  # Sub-MP panel correlation
  panel_features <- sub_labels_u
  panel_features <- panel_features[panel_features %in% colnames(refined_ucell)]

  common_cells <- Reduce(intersect, list(rownames(refined_ucell), Cells(pdos_merged)))
  ...
  panel_samples <- pdos_merged$orig.ident[match(rownames(panel_scores), Cells(pdos_merged))]
  panel_cor <- compute_fisher_mean_cor(panel_scores, panel_samples, min_cells = 10)

  panel_ids <- colnames(panel_cor)
  rownames(panel_cor) <- panel_ids
  colnames(panel_cor) <- panel_ids
  panel_split <- factor(rep(mp, length(panel_ids)))
...
dev.off()
cat("Saved:", file.path(outdir_live, "figures", "mp_splitting_diagnostics.pdf"), "\n")

# ============================================================================
# 9. Correlation heatmap (Fisher-Z per-sample meta-analysis)
# ============================================================================
...
mod_mat       <- t(module_scores)

samples_vec <- pdos_merged$orig.ident[match(colnames(mod_mat), Cells(pdos_merged))]
...
saveRDS(list(mean_rho = mean_rho, p_values = p_vals, samples = samples),
        file.path(outdir_ephemeral, "refined_mp_correlation_matrices.rds"))
write.csv(mean_rho,
          file.path(outdir_live, "tables", "refined_mp_correlation_mean_rho.csv"))
cat("Saved: correlation matrices and table\n")
...
ha_top <- HeatmapAnnotation(
  `Sub-MP` = submp_factor,
  col = list(`Sub-MP` = parent_mp_cols),
  show_annotation_name = TRUE, annotation_name_side = "left",
  show_legend = FALSE)

split_factor <- factor(mp_to_parent, levels = unique(mp_to_parent))
col_titles <- levels(split_factor)
...
pdf(file.path(outdir_live, "figures", "refined_mp_correlation_heatmap.pdf"),
...
dev.off()
cat("Saved:", file.path(outdir_live, "figures", "refined_mp_correlation_heatmap.pdf"), "\n")

# Unsupervised correlation heatmap
pdf(file.path(outdir_live, "figures", "refined_mp_correlation_heatmap_unsupervised.pdf"),
    width = 20, height = 20, useDingbats = FALSE)
...
dev.off()
cat("Saved:", file.path(outdir_live, "figures", "refined_mp_correlation_heatmap_unsupervised.pdf"), "\n")

# Free Seurat object
rm(pdos_merged); invisible(gc())
...
####################
mp_list  <- lapply(refined_genes_final[refined_mps_ordered], unique)
####################
mp_jn    <- names(mp_list)
...
saveRDS(list(jaccard = jaccard_mat, overlap_n = overlap_n_mat,
             p_values = pval_mat, padj = padj_mat),
        file.path(outdir_ephemeral, "refined_mp_jaccard_matrices.rds"))
write.csv(jaccard_mat,
          file.path(outdir_live, "tables", "refined_mp_jaccard_index.csv"))
cat("Saved: jaccard matrices and table\n")
...
pdf(file.path(outdir_live, "figures", "refined_mp_jaccard_heatmap.pdf"),
    width = 20, height = 20, useDingbats = FALSE)
...
dev.off()
cat("Saved:", file.path(outdir_live, "figures", "refined_mp_jaccard_heatmap.pdf"), "\n")
